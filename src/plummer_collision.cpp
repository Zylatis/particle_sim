...
int main ( int argc, char *argv[] ){
	...
	mass = 1./n;
	int step(0), file_n(0);
	current_dtype dt(0.1), t(0.), totalE(0.), tmax(config.tmax);//tmax( vm["tmax"].as<int>() );

	vector<current_dtype> strided_pos(3*n,0.), strided_vel(3*n,0.);
  vector< atomic<current_dtype> > strided_force(3*n);
